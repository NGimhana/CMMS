<template>
    ...
    <div>
        ...
        <div class="ui tiny sch modal">
            ...
            <div class="content">
                <p>You are going to Remove a Maintenance Job Task. Notifications and other features will be removed
                    permanently and This action will not be revert back !!</p>
                <h3 style="color: red">Are you Sure to Remove Job <span><strong>{{this.selectedJob.id}}</strong></span>
                    Completely from System ?</h3>
            </div>
                    ...
                    <div class="active content">
                        <form class="ui form">
                            <div style="padding-left: 7px; padding-bottom: 3px">
                                <div v-show="!this.isCompleted" class="ui  green button "
                                     v-on:click="completeJobTask()"><i class="checkmark icon"></i>COMPLETE
                                </div>
                            </div>
                            <!--Asset Info-->
    ...
    export default {
        ...
        data() {
            return {
                ...
                url:'http://cmms.com/api/scheduledjobs',
                current_page :'',
                last_page: '',
                next_page_url:'',
                prev_page_url:'',
        ...
        methods: {
            ...
            getSeleselectedJob: function (job) {
                ...
                this.$http.get('http://cmms.com/api/asset/' + this.selectedJob.asset_id).then(response => {
                    this.selectedAsset = response.body;

                    this.$http.get('http://cmms.com/api/sector/building/' + this.selectedAsset.sector_id).then(response => {
                        this.selectedBuilding = response.body;

                        //Create Map
                        this.createmap(this.selectedBuilding.description);

                        this.$http.get('http://cmms.com/api/sector/' + this.selectedAsset.sector_id).then(response => {
                            this.selectedSector = response.body;

                            this.$http.get('http://cmms.com/api/user/' + this.selectedJob.created_user_id).then(response => {
                                this.createdUser = response.body;

                                this.$http.get('http://cmms.com/api/user/' + this.selectedJob.Assigned_Person_id).then(response => {
            ...
            deleteJobTask: function () {
                this.$http.delete('http://cmms.com/api/scheduledjob/' + this.selectedJob.id).then(response => {
            ...
            completeJobTask: function () {
                this.getToday();
                this.$http.put('http://cmms.com/api/scheduledjob/' + this.selectedJob.id,
                    {
